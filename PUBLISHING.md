...
| 2026-05-27 | 0.0.1 | Launchpad PPA | ⏳ building | Source v1 rejected (orig.tar.gz captured Cargo.toml in patched state). Source v2 (0.0.1-1~noble2) accepted ~19:25 IST after fix: drop `--locked` + delete Cargo.lock pre-build (Ubuntu noble's cargo 1.75 can't parse upstream lockfile v4). GPG-signed by maintainer key `B5FDA553...A11731`. Launchpad currently building amd64 + arm64 binaries. |
| 2026-05-27 | 0.0.1 | Snap Store | ⚠ stuck | Snap name `ancilla` registered under publisher `ancillalive`. Initial Snapcraft Build Service (GitHub-connected): builds completed on Launchpad but artifacts never auto-uploaded to store (the snapcraft.io UI showed builds as "In progress" indefinitely; logs on Launchpad showed `Packed ancilla_0.0.1_amd64.snap` cleanly). Switched to local `snapcraft remote-build` — that also hung mid-upload. Currently parked. Squat protection via name registration is sufficient pre-launch; binary publication can be revisited later. |
